    ...
    <div class="main-wrapper main-wrapper-1">
      ...
      <div class="main-content">
        <section class="section">
          ...
          <?php
              $qs = mysqli_query($conn,"SELECT *FROM customers,companies WHERE customers.reserveStatus = 'Approved' AND companies.id=customers.companyId AND companies.id = {$_SESSION['spacecompanies']}");
                      ...
                      <div class="row ">
                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 pr-0 pt-3">
                          <div class="card-content">
                            ...
                            <?php
                              $findRev = mysqli_query($conn,"SELECT SUM(offices.price) as summation FROM offices,customers,companies WHERE offices.id = customers.id AND offices.companyId=companies.id AND companies.id= {$_SESSION['spacecompanies']}") or die(mysqli_error($conn));
                              $rev = mysqli_fetch_array($findRev);
                            ?>
                            <h2 class="mb-3 font-18">$<?= $rev['summation']?></h2>
                                ...
                                while ($rows = mysqli_fetch_array($query)) {
                                   ?>
                                        <tr>
                                            ...
                                            <td><button onClick="deleteProperty('<?= $rows['id'];?>','<?= $rows['name'];?>')" id="deleteCom" style="color:white;" class="btn btn-flat btn-danger btn-sm">Delete</button></td>                                            
