<?php
    ...
    $sele = mysqli_query($conn,"SELECT *FROM customers WHERE reserveId='$id'") or die(mysqli_error($conn));
       ...
       if ($fes['reserveStatus']=='Approve') {
           $check = mysqli_query($conn,"UPDATE customers SET reserveStatus='Approved', approvedAt='$approvedDay'") or die(mysqli_error($conn));
           ...
           if ($check) {
            ...
            $check1 = mysqli_query($conn,"UPDATE customers SET reserveStatus='Approve', approvedAt=''") or die(mysqli_error($conn));
            if ($check1) {
